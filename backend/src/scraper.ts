...
import { AnimeDetailsResponse, AnimeListResponse, EpisodeItem, PlayerSourceItem, SeasonItem, SeriesListItem } from "./types";
...
export function parseAnimeListFromHtml(html: string): SeriesListItem[] {
  ...
  $("article.post").each((_, el) => {
    ...
    if (imgEl && imgEl.length) {
      ...
      if (!img) {
        const attribs: Record<string, string> = (imgEl.get(0) as any)?.attribs || {};
        ...
        const scored = urlCandidates
          ...
          .sort((a, b) => b.s - a.s);
        if (scored.length) img = scored[0].u;
      }
    }
    ...
    if (img) {
      try { img = new URL(img, abs).toString(); } catch {}
    }
    ...
    if (typeof data === "object" && data) {
      ...
      try {
        ...
        if (parsed && typeof parsed === "object") {
          ...
          else if (typeof parsed.data === "string") html = parsed.data;
...
export async function fetchEpisodePlayers(episodeUrl: string): Promise<PlayerSourceItem[]> {
